...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!rsCreatorPk_b64)     return fail("A: rsCreatorPk_b64 missing", new Error("rsCreatorPk_b64 missing"), 400);

    console.log("[enc-upload] A ok");
  } catch (e) { return fail("A parse formData", e); }
  ...
  } catch (e) { return fail("B read/crypto init", e); }
  ...
  try {
    ...
    nonceBase = sodium.randombytes_buf(sodium.crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);
    ...
    for (let off = 0, i = 0; off < plainBuf.length; i++) {
      ...
      const cipher = sodium.crypto_aead_xchacha20poly1305_ietf_encrypt(
        plainBuf.subarray(off, end), aad, null, nonce, DEK
      );
  ...
  } catch (e) { return fail("C encrypt/hash", e); }
  ...
  } catch (e) { return fail("D kms/sealed-box", e); }
  ...
  } catch (e) { return fail("E pinata upload", e); }
