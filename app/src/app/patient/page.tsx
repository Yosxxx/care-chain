...
export default function PatientsPage() {
  ...
  function useWalletDid() {
    try {
      ...
      if (didStr.length > MAX_DID_LEN) throw new Error("Derived DID exceeds max length");
      setDid(didStr);
      setErr("");
    } catch (e: any) {
  ...
  function createDidKey() {
    try {
      ...
      if (didStr.length > MAX_DID_LEN) throw new Error("DID exceeds max length");
      setDid(didStr);
      ...
      try {
        ...
        localStorage.setItem("carechain_didkey_ed25519", JSON.stringify(payload));
      } catch { }
      setErr("");
    } catch (e: any) {
      setErr(e?.message ?? String(e));
  ...
  function parseHexOrThrow(hex: string) {
    ...
    if (!/^[0-9a-fA-F]{64}$/.test(clean)) throw new Error("id_hash hex must be 64 hex chars (32 bytes)");
      ...
      <div className="grid gap-3">
        ...
        <div className="grid gap-2 rounded border p-3">
          ...
          <p className="text-xs opacity-70">
            DID is a public identifier. Private keys stay in wallet/device/Vault.
          </p>
        </div>
        ...
        <div className="grid gap-2 rounded border p-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {idHashHex && (
              <span className="text-xs opacity-70">
                {idHashHex.slice(0, 10)}…{idHashHex.slice(-10)} ({idHashHex.replace(/^0x/i, "").length / 2} bytes)
              </span>
            )}
          </div>
          <input
          ...
          />
          <p className="text-xs opacity-70">
            Privacy: raw ID never leaves your browser; only the 32-byte digest is sent.
          </p>
        </div>
        ...
        <div className="space-y-3">
          {patients.map((p) => (
            <div key={p.pubkey} className="rounded border p-3 text-sm">
              ...
              <div><b>Created At:</b> {new Date(p.createdAt * 1000).toLocaleString(undefined, { hour12: false })}</div>
            </div>
          ))}
