...
export default function PatientsPage() {
    ...
    const [sig, setSig] = useState(""); const [err, setErr] = useState("");
    ...
    const patientPk = useMemo(() => { try { return new PublicKey(patientStr.trim()); } catch { return null; } }, [patientStr]);

    const patientPda = useMemo(() => (patientPk ? findPatientPda(programId, patientPk) : null), [programId, patientPk]);
    const seqPda = useMemo(() => (patientPk ? findPatientSeqPda(programId, patientPk) : null), [programId, patientPk]);
    ...
    const upsert = async () => {
        ...
        try {
            ...
            if (d.length > MAX_DID_LEN) throw new Error(`DID max ${MAX_DID_LEN} chars`);
    ...
    return (
        <main className="mx-auto max-w-2xl p-6 space-y-6">
            ...
            <div className="grid gap-3">
                <input className="rounded border px-3 py-2 font-mono text-sm"
                ...
                <div className="grid gap-2 rounded border p-3">
                    ...
                    <input className="rounded border px-3 py-2 text-sm"
                        ...
                        value={idSource} onChange={e => setIdSource(e.target.value)} />
                    <button onClick={computeHash} className="rounded border px-3 py-1 w-fit text-xs">Hash to 32-byte</button>
                    <input className="rounded border px-3 py-2 font-mono text-xs"
                        placeholder="or paste id_hash hex (0x… 64 hex chars)"
                        value={idHashHex} onChange={e => setIdHashHex(e.target.value)} />
                </div>

                <button onClick={upsert} disabled={!program || !patientPk}
                ...
                <div className="space-y-3">
                    {patients.map(p => (
                        <div key={p.pubkey} className="rounded border p-3 text-sm">
                            <div><b>Patient PDA:</b> <span className="font-mono">{p.pubkey}</span></div>
                            <div><b>id_hash:</b> <span className="font-mono break-all">{p.idHashHex}</span></div>
                            <div><b>DID:</b> {p.did}</div>
                            <div><b>Created At:</b> {new Date(p.createdAt * 1000).toLocaleString()}</div>
                        </div>
                    ))}
                    {!loading && patients.length === 0 && <p className="text-sm">No patients yet.</p>}
                </div>
            </section>
        </main>
    );
